...
transactionRoute.post("/add-transaction", async (req, res) => {
  try {
    ...
    if (req.body.action === "assign" && req.body.ticketNumber !== "") {
      ...
      const update = await productService.EditProductById(
        req.body.productId,
        product
      );
      delete req.body.action;
      try {
        await transactionService.EditTransaction(req.body.productId);
      ...
      product.status = req.body.action;
      result = await productService.EditProductById(
        req.body.productId,
        product
      );
    }
